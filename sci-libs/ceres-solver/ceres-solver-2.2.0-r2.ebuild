...
src_prepare() {
	...
	# eigen-5 ships an Eigen3ConfigVersion.cmake that rejects cross-major
	# version requests, but ceres-2.2.0 builds fine against eigen-5
	sed -e 's|find_package(Eigen3 3.3 REQUIRED)|find_package(Eigen3 REQUIRED)|' \
		-i CMakeLists.txt || die
	grep -q 'find_package(Eigen3 REQUIRED)' CMakeLists.txt || die "Eigen3 find_package sed failed"
}
